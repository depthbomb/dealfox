...
func (_q *ObservationQuery) QueryTarget() *TargetQuery {
...
func (_q *ObservationQuery) QueryEvents() *EventQuery {
...
func (_q *ObservationQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *ObservationQuery) FirstIDX(ctx context.Context) string {
...
func (_q *ObservationQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *ObservationQuery) OnlyIDX(ctx context.Context) string {
...
func (_q *ObservationQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (_q *ObservationQuery) IDsX(ctx context.Context) []string {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *ObservationQuery) WithTarget(opts ...func(*TargetQuery)) *ObservationQuery {
	query := (&TargetClient{config: _q.config}).Query()
...
func (_q *ObservationQuery) WithEvents(opts ...func(*EventQuery)) *ObservationQuery {
	query := (&EventClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(observation.FieldTargetID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Observation.Query().
//		Select(observation.FieldTargetID).
//		Scan(ctx, &v)
...
func (_q *ObservationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Observation, error) {
	...
	if query := _q.withTarget; query != nil {
		if err := _q.loadTarget(ctx, query, nodes, nil,
			func(n *Observation, e *Target) { n.Edges.Target = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withEvents; query != nil {
		if err := _q.loadEvents(ctx, query, nodes,
			func(n *Observation) { n.Edges.Events = []*Event{} },
			func(n *Observation, e *Event) { n.Edges.Events = append(n.Edges.Events, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ObservationQuery) loadTarget(ctx context.Context, query *TargetQuery, nodes []*Observation, init func(*Observation), assign func(*Observation, *Target)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Observation)
	for i := range nodes {
		fk := nodes[i].TargetID
...
func (_q *ObservationQuery) loadEvents(ctx context.Context, query *EventQuery, nodes []*Observation, init func(*Observation), assign func(*Observation, *Event)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(event.FieldObservationID)
	}
	query.Where(predicate.Event(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(observation.EventsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *ObservationQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(observation.Table, observation.Columns, sqlgraph.NewFieldSpec(observation.FieldID, field.TypeString))
	_spec.From = _q.sql
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withTarget != nil {
			_spec.Node.AddColumnOnce(observation.FieldTargetID)
		}
	}
...
func (_q *ObservationQuery) ForUpdate(opts ...sql.LockOption) *ObservationQuery {
...
func (_q *ObservationQuery) ForShare(opts ...sql.LockOption) *ObservationQuery {
	if _q.driver.Dialect() == dialect.Postgres {
		_q.Unique(false)
	}
	_q.modifiers = append(_q.modifiers, func(s *sql.Selector) {
